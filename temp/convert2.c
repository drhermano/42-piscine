...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/15 12:28:41 by drocha-h          #+#    #+#             */
/*   Updated: 2026/03/15 13:09:29 by drocha-h         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_chunk_len(t_convert_ctx *ctx)
{
	if (ctx->i == 0)
...
static void	init_ctx(t_convert_ctx *ctx, char *num, t_lang lang)
{
...
static int	process_nonzero_chunk(t_dict *dict, int size,
			char *chunk, t_convert_ctx *ctx)
{
	...
	if (ctx->lang == LANG_PT && ft_strcmp(chunk, "1") == 0
		...
		skip_chunk = 1;
	if (!skip_chunk && !convert_chunk(dict, size, chunk,
			&ctx->first, ctx->lang))
		return (0);
